...
sólo en el futuro), pero más generalmente buscamos modelar patrones o estructuras
interesantes en los datos.
...
- Quisiéramos obtener una respuesta barata, rápida y con suficiente precisión.
...
- Quisiéramos mejorar el desempeño de los expertos o de reglas simples utilizando
...
```{r funcgraf}
graficar.digitos <- function(data.frame){
  matriz.digitos <- lapply(1:nrow(data.frame), function(x){ 
    	matrix(data.frame[x, 257:2], 16, 16)[16:1, ]
    })
	image(Reduce("rbind", matriz.digitos), 
    col = terrain.colors(30))
	text(seq(0,1,1/10) + 0.05, 0.05, label = data.frame[, 1], cex = 1.5)
}
```

```{r grafdigitos}
library(ElemStatLearn)
muestra.1 <- zip.train[sample(1:nrow(zip.train), 10), ]
graficar.digitos(muestra.1)
muestra.2 <- zip.train[sample(1:nrow(zip.train), 10), ]
graficar.digitos(muestra.2)
```
...
ggplot(datos, 
  ...
  geom_smooth(se =FALSE, colour='red', size=1.1, 
    span=0.5, method='loess',family='symmetric')
```
...
```{r}
autos.1 <- Auto[, c('mpg','cylinders','displacement', 'horsepower','acceleration')]
comps.autos <- princomp(autos.1, cor = T)
clust.1 <- hclust(dist(comps.autos$scores[,1:2]))
autos.1$grupo <- cutree(clust.1, k=4)
autos.1$Comp.1 <- comps.autos$scores[,1]
autos.1$Comp.2 <- comps.autos$scores[,2]
autos.1$nombre <- Auto$name
ggplot(autos.1, aes(x=Comp.1, y=Comp.2, colour=factor(grupo), label=nombre)) +
  geom_point()
```
...
```{r}
head(subset(autos.1, grupo==1))
head(subset(autos.1, grupo==2))
head(subset(autos.1, grupo==3))
head(subset(autos.1, grupo==4))
```
...
de coches.
Entonces agregamos una error aleatorio $\epsilon$, con media cero, y que
*{\em **no contiene** información acerca de $X$ (por eso es independiente de $X$) que
junto con $f(X)$ determina $Y$
...
### Aprendizaje supervisado
Bajo los supuestos del modelo $Y=f(X)+\epsilon$, {\em aprender de los datos}
...
### Estimar f y hacer predicciones
...
En nuestro ejemplo anterior, podríamos construir, por ejemplo
...
- La diferencia $f(X) - \hat{f}(X)$ está asociada a {\em error reducible}, pues
depende de qué tan bien estimemos $f(X)$ con $\hat{f}(X)$
- El error aleatorio $\epsilon$, asociado a {\em error irreducible}
...
del error irreducible, pues este se debe a aleatoriedad en el fenómeno.
\end{shaded}
...
donde $\epsilon$ es una término de error aleatorio que no depende de $X_1,\ldots, X_p$, y
que tiene valor esperado $\textrm{E}(\epsilon)=0$.
...
library(arm)
library(Hmisc)
mod.1 <- loess(mpg~year+weight, data=Auto, family='symmetric', degree=1, 
  span=0.15)
...
Escribimos también 
$$\underbar{X} =  \left ( \begin{array}{cccc}
x_{1} & x_{12} & \ldots  & x_{1p} \\
x_{21} & x_{22} & \ldots  & x_{2p} \\
\vdots & \vdots &   &  \vdots \\
x_{n1} & x_{n2} & \ldots  & x_{np} \\
 \end{array} \right)$$
y $$\underbar{y} =(y_1,y_2, \ldots, y_n)^t.$$
...
Por otro lado, estos métodos nos protegen generalmente de {\em sobreajustar}
...
Así que el modelo rojo se `desempeña` mejor que el gris {\em sobre la muestra
