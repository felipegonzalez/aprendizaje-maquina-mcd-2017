...
¿Cuántos casos tenemos para hacer prueba?

```{block2, type='comentario'}
Cuando el tamaño de muestra $n$ es grande, el porcentaje esperado de casos que
no están en la muestra bootstrap es de 37\%
```
...
Para un bosque aleatorio con árboles $T_1^*, T_2^*, \ldots, T_B^*$, y conjunto de 
entrenamiento original
${\mathcal L} =\{(x^(i), y^(i)\}_{i=1}^n$, la estimación OOB del error de predicción del bosque es
