...
$$\hat{Err}_{oob} = \frac{1}{n} (y^{(i)} - y^_{oob}^(i))^2$$
2. Clasificación (error de clasificación)
$$\hat{Err}_{oob} = \frac{1}{n} I(y^{(i)} = y^_{oob}^(i))$$
...
- No podemos "quitar" la variable en el ajuste de los árboles, pues entonces otras variables
...
- Pero podemos permutar a la hora de probar (no entrenar), rompiendo la relación que
hay entre $X_k$ y la variable respuesta.
- Este método funciona bien con bosques aleatorios, pues el proceso de aleatorización
en la selección de variables aminora el problema de enmascaramiento que notamos en árboles.
