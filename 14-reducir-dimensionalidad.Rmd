...
normalizar <- function(x){
  norma <- sqrt(sum(x^2))
  x_norm <- ifelse(norma > 0,  x/norma, 0)
  x_norm
}
v <- normalizar(v)
...
más grande para los que hay solución de \@ref(eq:valor-propio-derecho) y \@ref(eq:valor-propio-izquierdo) y
...
valor singular más grande después de $\sigma_1$, de forma que tenemos $\sigma_1^2 \leq \sigma_2^2$.
...
sum((X_arr - X_arr_2)^2)
```
Pero podemos calcular $X_arr_2$ en forma matricial, haciendo
```{r}
...
de predicción de Netflix (una de las componentes de las soluciones ganadaras
...
tiene bajo interés en una película que no ha visto. Podemos también poner la media
de cada usuario, indicando que no tiene preferencia clara por esa película. Intentemos la segunda opcion
...
```{r}
qplot(svd_parcial$u[,2]*sqrt(svd_parcial$d[2]))
qplot(svd_parcial$v[,2]*sqrt(svd_parcial$d[2]))
```
...
```{r}
qplot(svd_parcial$u[,1]*sqrt(svd_parcial$d[1]))
qplot(svd_parcial$v[,1]*sqrt(svd_parcial$d[1]))
```
