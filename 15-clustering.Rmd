...
solución $C_1^*,C_2^*,\ldots,C_K^*$ de \#ref:(eq:kmedias-original), podemos
...
ampliado, 
podemos desarrollar una heurística notando que si 
...
$$
pues los clusters $C_1,\ldots C_K$ se forman encontrando el punto $m_j$ más cercano.
...
problema original por el problema ampliado. Y ahora podemos repetir: agrupar en clusters,
...
3.Nos detenemos cuando los centroides se quedan casi fijos de una iteración a la siguiente (y en consecuencia la cantidad a minimizar no decrece más).
```
...
```{r}
sum(k_medias$withinss)
k_medias$tot.withinss
```

nos da el valor de \@ref(eq:kmedias-original) para la solución obtenido.

- El algoritmo de default para R permite correrlo repetidamente para distintos centroides aleatorios, y nos da la mejor solución. El algoritmo
usado es el de Hartigan-Wong, que es más eficiente que la implementación
simple que vimos arriba.
...
A veces el punto de corte no es tan claro, pero la gráfica sigue siendo útil
```{r}
set.seed(2800)
...
Las soluciones son
```{r}
grupos.df <- lapply(ajustes.km[1:8], function(aj){ 
  data.frame(num=max(aj$cluster), cluster= aj$cluster, id= 1:length(aj$cluster))}) %>% rbind_all
grupo.df.2 <- left_join(grupos.df, quakes_1)
ggplot(grupo.df.2, aes(x=lat, y=long, colour=factor(cluster))) + facet_wrap(~num) +
  geom_point()
```
...
```{r}
dat.2 <- dat %>% gather(var, valor, gvjbevn:sbcwkfm) %>% left_join(nombres) %>%
  dplyr::select(-var) %>%
  group_by(nombre) %>%
