...
Sin embargo, escogiendo medidaas $W$ particulares es posible construir algoritmos con desempeño razonable. En primer lugar, suponemos que $W(C)$ está dado por
...
euclideanas al cuadrado dentro del cluster. El problema

$$\min_{C_1,\ldots, C_K} \sum_{k=1}^K \frac{1}{|C_k|}\sum_{i,j\in C_k} ||x_i-x_j||^2$$ 

todavía es demasiado difícil. Sin embargo, se puede demostrar (usando pitágoras) que
...
ggplot(quakes_1, aes(x=lat, y=long)) + geom_point()
```
...
ggplot(quakes_1, aes(x=lat, y=long)) + geom_point() +
  geom_point(data = centros_1, aes(x=lat, y=long), size=7, colour='red')
```
...
ggplot(quakes.1, aes(x=lat, y=long)) + geom_point() +
  geom_point(data = centros, aes(x=lat, y=long), size=7, colour='red')
```
