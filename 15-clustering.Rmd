...
Sin embargo, escogiendo medidaas $W$ particulares es posible construir algoritmos con desempeño razonable. 


En primer lugar, suponemos que $W(C)$ está dado por
...
todavía es demasiado difícil. Sin embargo, se puede demostrar que este
problema es equivalente a minimizar

$$W(C_k)=2\sum_{i\in C_k} ||x_i-\bar{x}_k||^2,$$

donde $\bar{x}_k=\frac{1}{|C_k|}\sum_{i\in C_k} x_i$ es el centroide del grupo $C_k$.
Esto se puede demostrar usando que

$$\sum_{i\in C_k} ||x_i-\bar{x}_k||^2 = \sum_{i\in
C_k} (x_i-\frac{1}{n}\sum_m x_m)^t
(x_i-\frac{1}{n}\sum_l x_l) = $$
