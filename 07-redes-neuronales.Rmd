...
- $a^{(l)}_j$ el valor que toma la unidad $j$ de la capa $l$.
- En particular, ponemos $a^{(l)}_j = x_j$, que son los valores de las entradas (primera capa)
- Escribimos también $a^{(l)}_0=1$ para lidiar con los sesgos.
...
de pérdida, y sin regularización**.

Recordamos nuestra notación para la función logística (o sigmoide):
...
Veamos dónde aparece $\theta_{j,k}^{(l)}$ en la gráfica de la red:

$$ \cdots a^{(l-1)}_k \xrightarrow{\theta_{j,k}^{(l)}} a^{(l)}_j  \cdots \rightarrow  D$$

Entonces, por la regla de la cadena,  podemos escribir
$$\frac{\partial D}{\partial \theta_{j,k}^{(l)}} =
...
y esta última expresión podemos calcularla pues sólo requiere la derivada de $h$ y
los valores de los nodos obtenidos en la pasada de feed-forward.

Así que sólo nos queda calcular las parciales
$$\frac{\partial D}{\partial a_j^{(l)}}$$

Para obtener una fórmula recursiva para esta cantidad (hacia atrás), consideramos
la estructura de la red en $a_j^{(l)}$:
...
Ahora nótese que, aplicando regla de la cadena (ojo: queremos obtener
...
$z_s^{(l+1)}$ para reducir el error $D$. Cuando este valor es grande,
implica que la 'contribución' al error del valor actual de $z_s^{(l+1)}$ es grande.
...
Tenemos que si $l=2,\ldots,L-1$, entonces podemos escribir
...
$$\delta^{(L)}=p-y.$$
Y además es fácil ver que ($\theta_{*}^{(l+1)}$ *sin* la columna correspondiente al sesgo):

$$\delta^{(l)}=\left( \theta_{*}^{(l+1)}    \right)^t\delta^{(l+1)} \circ h'(z^{(l)})$$
...
Ahora todo ya está calculado. Lo interesante es que las $\delta^()$ se calculan
de manera recursiva.
...
2. Calculamos $a^{(2)},a^{(3)},\ldots, a^{(L)}$ usando feed forward.
3. Calculamos $\delta^{(L)}=a^{ (L)}-y^i$, y luego
$\delta^{(L-1)},\ldots, \delta^{(2)}$ como en la sección anterior.
4. Acumulamos
$\Delta_{j,k}^{(l)}=\Delta_{j,k}^{(l)} + \delta_j^{(l)}a_k^{(l-1)}$.


Finalmente, ponemos, si $k\neq 0$,
$$D_{j,k}^{(l)} = \frac{2}{n}\Delta_{j,k}^{(l)} + \lambda\theta_{j,k}^{(l)}$$
y si $k=0$,
