...
```{r}
dat_a <- data.frame(x=x, a.1=a.1, a.2=a.2)
dat_a_2 <- dat_a %>% gather(variable, valor, a.1:a.2)
ggplot(dat_a_2, aes(x=x, y=valor, colour=variable, group=variable)) + geom_line()
```

Si las escalamos y sumamos, obtenemos
```{r}
dat_a <- data.frame(x=x, a.1=-4+12*a.1, a.2=-12*a.2, suma=-4+12*a.1-12*a.2)
dat_a_2 <- dat_a %>% gather(variable, valor, a.1:a.2)
ggplot(dat_a_2, aes(x=x, y=valor, colour=variable, group=variable)) + geom_line()
```
...
```{r}
dat_2 <- data.frame(x, p2=h(-4 + 12*a.1 - 12*a.2))
...
Por ejemplo:
```{r}
dev <- devianza_fun(x.1, g.1) # crea función dev
...
devianza_reg <- function(x, y, lambda){
   ...
   devianza <- function(beta){
         p <- feed_fow(beta, x)
      - 2 * mean(y*log(p) + (1-y)*log(1-p)) + lambda*sum(beta^2)
   }
  devianza
}
```
...
```{r}
dev_r <- devianza_reg(x.1, g.1, 0.001) # crea función dev
set.seed(5)
...
#### Ejercicio {-}
Muestra por ejemplo que es posible capturar interacciones entre las
variables $x_i$ de entrada: considerando dos entradas $x_1,x_2$ y dos
variables derivadas $a_1,a_2$, construye una salida
$g=1$ cuando $x_1=1,x_2=0$ o $x_1=0,x_2=1$, pero $g=0$ en otro caso.
...
Nótese que en estos problemas no buscamos el mínimo global, sino un mínimo
local de buen desempeño. 

Más adelante veremos como usar descenso en gradiente (que en este contexto
se hace a través de un cálculo que se llama *back-propagation*) para encontrar 
mínimos de la devianza para una red.
También es posible usar métodos más avanzados (como el paquete *nnet*) de optimización,
pero la adaptablidad de descenso en gradiente le da algunas ventajas sobre estos métodos
más avanzados.

Es importante
no comenzar valores de los pesos grandes, pues las funciones logísticas pueden
quedar en regiones planas donde la minimización es lenta. Nótese adicionalmente
que las arquitecturas de las redes que utilizamos son simétricas en cuanto a las
unidades ocultas (no hay nada que distinga a una de otra), de forma que si comenzamos,
por ejemplo, con pesos igual a cero, la red y los pesos son simétricos. Esto implica
que cuando apliquemos un método de minimización, la simetría se preserva, y todas
las unidades ocultas aprenden lo mismo!
...
- Verificar convergencia del algoritmo a un mínimo local.
- Predecir usando el modelo ajustado. Para problemas chicos, podemos repetir
el proceso con varios pesos iniciales aleatorios y promediar los modelos para clasificar.
