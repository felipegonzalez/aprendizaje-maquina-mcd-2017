...
3, 0.1))
grid_graf <- grid_graf %>%  mutate(val = h(z_1,z_2))
ggplot(grid_graf, aes(x = z_1, y = z_2, z = val)) + 
...
```{r}
h_grad <- function(z_1,z_2){
  c(2*z_1 - z_2, 2*z_2 - z_1)
}
```
...
ggplot(grid_graf, aes(x=z_1, y=z_2, z=val)) +
  ...
  geom_segment(aes(x=0.0, xend=0.0-eta*grad.1[1], y=-2,
     yend=-2-eta*grad.1[2]),
...
Ya aplicamos descenso en gradiente:
