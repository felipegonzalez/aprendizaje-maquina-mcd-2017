...
seleccion_ajuste <- function(...){
  ...
  modelo <- cv.glmnet(y=y, x= x[,vars_selec], alpha=0, family='binomial',
                    lambda = c(0.5,10), type.measure = 'class')
  error_validacion <- modelo$cvm[which.min(modelo$cvm)]
  y_p <- rbinom(1000, 1, 0.5)
  x_p <- matrix(rnorm(1000*500,0,1), 1000, 500)
  preds_p <- predict.cv.glmnet(modelo, newx = x_p[, vars_selec], s='lambda.min')
...
seleccion_ajuste_correcto <- function(...){
  ...
  modelo <- glmnet(y=y, x= x[,vars_selec], alpha=0, family='binomial',
                    lambda = 0.5)
  preds_p <- predict(modelo, newx = x_p[, vars_selec], s='lambda.min')
  error_prueba <- mean((preds_p > 0) == y_p)
  c('error_valida'=error_validacion, 'error_prueba'=error_prueba)
}
