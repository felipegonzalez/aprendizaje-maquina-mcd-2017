...
\begin{equation}
\min_{(\beta_m,b_m)} \sum_{i=1}^N L(y^{(i)}, \sum_{k=1}^M T_k(x^{(i)}))
\end{equation}
...
que es el error para el caso $i$ y el modelo $f_{m-1}$, entonces
reescribimos el problema anterior como
r_{m-1}^{(i)}
\begin{equation}
\min_{(\beta,b)} \sum_{i=1}^N ( r_{m-1}^{(i)} - T(x^{(i)}))^2
\end{equation}
...
agregar_arbol <- function(arboles_fsam, dat, plot=TRUE){
  ...
  g_res <- ggplot(dat, aes(x = x)) + geom_line(aes(y=preds_nuevo)) +
    geom_point(aes(y=res)) + labs(title = 'Residuales') + ylim(c(-6,6))
...
$$(\beta_m, b_m) = argmin_{(\beta,b)} -2\sum_{i=1}^N \log (1+ e^{-y^{(i)}(f_{m-1}(x^{(i)}) + T(x^{(i)})})$$

Nótese que sólo optimizamos con respecto a $T$, así que
podemos escribir

$$(\beta_m, b_m) = argmin_{(\beta,b)} -2\sum_{i=1}^N \log (1+ d_{m,i}e^{- T(x^{(i)})})$$
...
La idea de gradient boosting es replicar la idea del residual, y usar
árboles de regresión para resolver ****.
...
escogemos $\gamma$ (tamaño de paso) suficientemente chica y ponemos
$$f_m(x) = f_{m-1}(x)+\gamma T(x)$$
