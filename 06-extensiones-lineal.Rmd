# Extensiones de regresión e Intro a redes neuronales
...
2. El modelo tiene sesgo, en el sentido de que la estructura lineal
...
considerarlas como una {\em entrada derivada} de las entradas originales.
...
### Variables cualitativas y transformación de variables
...
variables de entrada. Una de las más comunes es para variables positivas:

```{block2, type='comentario'}
- Cuando una variable que toma valores positivos y recorre varios órdenes 
de magnitud, 
...
#### Ejemplo {-}
Supongamos que queremos incluir la variable que identifica la calidad de corte:

```{r}
diamonds_muestra %>% group_by(cut) %>% count
...
**Observaciones**:
- Nótese también que hay coeficiente para una de las clases, por lo que discutimos arriba. También podemos pensar que el coefi- ciente de esta clase es 0, y así comparamos con las otras clases.
...
ggplot(air[1:50,], aes(x = Temp, y = Ozone)) + geom_point() + geom_smooth(method = 'lm')
```
...
ggplot(air[1:50,], aes(x = Temp, y = Ozone, colour= cut(Wind, cuantiles))) + 
  geom_point() + geom_smooth(method = 'rlm', se = FALSE)
```
...
fácilmente podemos encontrar problemas numéricos. Una
 alternativa es usar *splines*, que son familias de funciones con buenas propiedades
...
```{r}
library(splines2)
splines_age  <- mSpline(seq(18,80, 0.2), 
...
```{r}
dat_wage <- ISLR::Wage 
splines_age  <- mSpline(dat_wage$age, 
                         knots = c(25, 35, 45, 65),
                         degree = 2) %>% data.frame
...
O podemos usar i-splines, por ejemplo:

```{r}
library(splines2)
splines_age  <- iSpline(seq(18,80, 0.2), 
                         knots = c(25, 35, 45, 65),
                         degree = 2)
matplot(splines_age, type='l')
``` 
