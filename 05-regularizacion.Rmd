...
sim_datos <- function(n, m, beta){
  ...
  mat_entrena <- matrix(rnorm(n*p, 0, 0.5), n, p) + rnorm(p) 
  mat_prueba <- matrix(rnorm(m*p, 0, 0.5), m, p) + rnorm(p) 
  p_entrena <- h(mat_entrena %*% beta) 
  p_prueba <- h(mat_prueba %*% beta) 
  y_entrena = rbinom(n, 1, p_entrena)
  y_prueba = rbinom(m, 1, p_prueba)
  dat_entrena <- as.data.frame(mat_entrena)
  dat_prueba <- as.data.frame(mat_prueba)
  dat_entrena$y <- y_entrena
  dat_prueba$y <- y_prueba
  dat_entrena$entrena <- TRUE
  dat_prueba$entrena <- FALSE
  datos <- bind_rows(dat_entrena, dat_prueba)
  datos
}
...
mod_restringido <- glmnet(x = x_e, y = dat_e$y, 
  ...
  lambda = 0.1)
beta_penalizado <- coef(mod.restringido)[-1] # quitar intercept
```
...
```{r}
p_entrena$prob_hat_pen <- h(x_e %*% as.numeric(beta.restr))
p_prueba$prob_hat_pen <- h(x_p %*% as.numeric(beta.restr))
```
Para los datos de entrenamiento:
```{r}
...
```{block2, type='comentario'}
$$\frac{\partial D_{\lambda}^2 (\beta)}{\partial\beta_j} = \frac{\partial D(\beta)}{\beta_j} + 2\lambda\beta_j$$
para $j=1,\ldots, p$, y 
